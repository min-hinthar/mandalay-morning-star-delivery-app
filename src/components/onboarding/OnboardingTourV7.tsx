...
/**
 * V7 Onboarding Tour - Motion-First Welcome Experience
 *
...
export interface OnboardingTourV7Props {
  /** Steps to display */
...
interface StepIllustrationV7Props {
  icon: keyof typeof ICON_MAP;
  isActive: boolean;
}

function StepIllustrationV7({ icon, isActive }: StepIllustrationV7Props) {
  ...
  return (
    <div className="relative w-48 h-48 flex items-center justify-center">
      ...
      <motion.div
        ...
        className="absolute inset-0 rounded-full bg-gradient-to-br from-v6-primary/20 to-v6-secondary/20"
      />
      <motion.div
        animate={
          isActive && shouldAnimate
        ...
        className="absolute inset-4 rounded-full bg-gradient-to-br from-v6-secondary/20 to-v6-green/20"
      />
        ...
        <>
          {[...Array(5)].map((_, i) => (
            <motion.div
              ...
              className="absolute w-3 h-3 rounded-full bg-v6-secondary"
            />
          ))}
        </>
      )}
...
interface ProgressDotsV7Props {
  total: number;
  current: number;
  onDotClick?: (index: number) => void;
}

function ProgressDotsV7({ total, current, onDotClick }: ProgressDotsV7Props) {
      ...
      {Array.from({ length: total }, (_, i) => (
        <motion.button
          ...
          className={cn(
            ...
            i === current ? "w-8 bg-v6-primary" : "w-2 bg-v6-surface-secondary",
...
export function OnboardingTourV7({
  steps = DEFAULT_STEPS,
  onComplete,
  onSkip,
  className,
    ...
    >
      ...
      <motion.button
        ...
        className={cn(
          ...
          "text-sm text-v6-text-secondary",
          "hover:bg-v6-surface-secondary transition-colors"
        )}
      >
        Skip
        <X className="w-4 h-4" />
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="flex justify-center mb-8">
              <StepIllustrationV7 icon={step.icon} isActive={true} />
            </div>
            ...
            <motion.h1
              ...
              className="text-2xl font-bold text-v6-text-primary mb-4"
            >
              {step.title}
            </motion.h1>
            ...
            <motion.p
              ...
              className="text-v6-text-secondary mb-8 leading-relaxed"
            >
              {step.description}
            </motion.p>
          </motion.div>
      ...
      {shouldAnimate && (
        <>
          <motion.div
            ...
            className="absolute bottom-20 left-10 w-20 h-20 rounded-full bg-v6-secondary/10"
          />
          <motion.div
            animate={{ y: [0, 15, 0], rotate: [0, -3, 0] }}
            transition={{ duration: 6, repeat: Infinity, delay: 1 }}
            className="absolute top-32 right-16 w-16 h-16 rounded-full bg-v6-primary/10"
          />
          <motion.div
            animate={{ y: [0, -10, 0], scale: [1, 1.1, 1] }}
            transition={{ duration: 5, repeat: Infinity, delay: 2 }}
            className="absolute top-1/4 left-1/4 w-8 h-8 rounded-full bg-v6-green/10"
          />
        </>
      )}
    </motion.div>
...
export default OnboardingTourV7;
