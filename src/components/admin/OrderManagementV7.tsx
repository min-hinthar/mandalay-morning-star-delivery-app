...
/**
 * V7 Order Management - Motion-First Drag & Drop
 *
...
export interface OrderManagementV7Props {
  /** Orders to display */
  orders: OrderItemV7[];
...
> = {
  pending: {
    ...
    borderColor: "border-v6-secondary/30",
  },
  confirmed: {
    label: "Confirmed",
    icon: CheckCircle2,
    color: "text-v6-accent-teal",
    bgColor: "bg-v6-accent-teal/10",
    borderColor: "border-v6-accent-teal/30",
  },
  preparing: {
    label: "Preparing",
    icon: Timer,
    color: "text-v6-accent-magenta",
    bgColor: "bg-v6-accent-magenta/10",
    borderColor: "border-v6-accent-magenta/30",
  },
  out_for_delivery: {
    ...
    borderColor: "border-v6-primary/30",
  },
  delivered: {
    label: "Delivered",
    icon: CheckCircle2,
    ...
    borderColor: "border-v6-green/30",
  },
  cancelled: {
    label: "Cancelled",
    icon: XCircle,
    ...
    borderColor: "border-v6-status-error/30",
  },
};
...
function OrderCardV7({
  ...
  return (
    ...
    >
      ...
      >
        <GripVertical className="w-4 h-4 text-v6-text-muted" />
      </motion.div>
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-mono text-sm font-semibold text-v6-text-primary">
              #{order.orderNumber}
            </p>
      ...
      <div className="space-y-2 mb-3">
        <div className="flex items-center gap-2 text-sm">
          <User className="w-4 h-4 text-v6-text-muted flex-shrink-0" />
          <span className="text-v6-text-primary truncate">
        ...
        {order.deliveryWindowStart && (
          <div className="flex items-center gap-2 text-sm">
            <Clock className="w-4 h-4 text-v6-text-muted flex-shrink-0" />
        ...
        {order.address && (
          <div className="flex items-center gap-2 text-sm">
            <MapPin className="w-4 h-4 text-v6-text-muted flex-shrink-0" />
            <span className="text-v6-text-secondary truncate">{order.address}</span>
          </div>
        )}
      </div>

      {/* Footer */}
      <div className="flex items-center justify-between pt-3 border-t border-v6-border/50">
        <div className="flex items-center gap-2">
          <Package className="w-4 h-4 text-v6-text-muted" />
          <span className="text-sm text-v6-text-secondary">
            ...
            <div className="flex gap-2">
              ...
              {onOrderClick && (
                <motion.button
                  ...
                  className="p-2 rounded-lg bg-v6-surface-secondary text-v6-text-secondary hover:text-v6-text-primary"
                >
      ...
      {order.driverName && order.status === "out_for_delivery" && (
        <motion.div
          ...
          className="absolute -top-2 -right-2 flex items-center gap-1 px-2 py-1 rounded-full bg-v6-primary text-white text-xs shadow-lg"
        >
...
interface KanbanColumnV7Props {
  status: OrderStatus;
  orders: OrderItemV7[];
  onStatusChange: (orderId: string, newStatus: OrderStatus) => Promise<void>;
  onOrderClick?: (orderId: string) => void;
  onReorder: (orders: OrderItemV7[]) => void;
}

function KanbanColumnV7({
  status,
  orders,
  onStatusChange,
  onOrderClick,
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <span className="font-semibold text-v6-text-primary">{config.label}</span>
        <motion.span
          key={orders.length}
      ...
      {/* Column content */}
      <div className="h-[calc(100vh-300px)] overflow-y-auto p-3 bg-v6-surface-secondary/50 rounded-b-xl">
        {orders.length === 0 ? (
          <motion.div
          ...
          >
            ...
            <p className="text-sm text-v6-text-muted">No orders</p>
          </motion.div>
        ) : (
          <Reorder.Group
            axis="y"
...
function ListRowV7({ order, index, onStatusChange, onOrderClick }: ListRowV7Props) {
  const { shouldAnimate, getSpring } = useAnimationPreferenceV7();
  const [isUpdating, setIsUpdating] = useState(false);
  ...
  return (
    <motion.div
      ...
      transition={{ ...getSpring(v7Spring.snappy), delay: index * 0.03 }}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="font-mono font-semibold text-v6-text-primary">
            #{order.orderNumber}
          </span>
          <span className={cn(
            "px-2 py-0.5 rounded-full text-xs font-medium",
        ...
        <p className="text-sm text-v6-text-secondary truncate">
          {order.customerName || order.customerEmail}
        </p>
      </div>
      ...
      <div className="text-right w-24">
        {order.deliveryWindowStart ? (
          ...
          <p className="text-sm text-v6-text-muted">—</p>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {onOrderClick && (
          <motion.button
            ...
            whileTap={shouldAnimate ? { scale: 0.95 } : undefined}
            onClick={() => onOrderClick(order.id)}
            className="p-2 rounded-lg text-v6-text-muted hover:text-v6-text-primary hover:bg-v6-surface-secondary"
          >
...
export function OrderManagementV7({
  orders,
  onStatusChange,
  onOrderClick,
  viewMode = "kanban",
  ...
  const ordersByStatus = useMemo(() => {
    const grouped: Record<OrderStatus, OrderItemV7[]> = {
      pending: [],
      confirmed: [],
      preparing: [],
      out_for_delivery: [],
  ...
  const handleReorder = useCallback(
    (status: OrderStatus) => (reorderedOrders: OrderItemV7[]) => {
    ...
    return (
      <div className={cn("space-y-4", className)}>
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className="h-24 rounded-xl bg-v6-surface-tertiary animate-pulse"
          />
        ))}
      </div>
    );
    ...
    return (
      <motion.div
        ...
        className={cn(
          ...
          "bg-v6-surface-secondary rounded-2xl",
          className
        )}
      >
        <motion.div
          ...
          className="p-4 rounded-full bg-v6-primary/10 mb-4"
        >
          <Package className="w-8 h-8 text-v6-primary" />
        </motion.div>
        <h3 className="text-lg font-semibold text-v6-text-primary mb-1">
          No orders yet
        </h3>
        <p className="text-v6-text-muted">Orders will appear here as they come in</p>
      </motion.div>
    );
  }
  ...
  return (
    <div className={cn("", className)}>
      {viewMode === "kanban" ? (
        ...
        <motion.div
          variants={shouldAnimate ? v7StaggerContainer(0.03, 0.1) : undefined}
          initial="hidden"
          animate="visible"
          className="space-y-3"
        >
...
export default OrderManagementV7;
