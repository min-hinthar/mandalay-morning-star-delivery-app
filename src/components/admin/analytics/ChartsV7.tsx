...
/**
 * V7 Animated Charts - Motion-First Data Visualization
 *
...
export interface ChartsV7Props {
  /** Chart data */
  data: ChartDataPoint[];
  /** Chart title */
  title: string;
...
function CustomTooltipV7({ active, payload, label, format }: CustomTooltipProps) {
  ...
  return (
    ...
    >
      <p className="text-xs text-v6-text-muted mb-1">{label}</p>
      {payload.map((entry, index) => (
        <p key={index} className="text-lg font-bold text-v6-text-primary">
...
function ChartHeaderV7({ title, type, trend, showTrend, onTypeChange }: ChartHeaderV7Props) {
  const { shouldAnimate } = useAnimationPreferenceV7();
  ...
  return (
    <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-3">
        ...
        >
          <Activity className="w-5 h-5 text-v6-primary" />
        </motion.div>
        <h3 className="text-lg font-bold text-v6-text-primary">{title}</h3>
      ...
      {/* Chart type toggle */}
      {onTypeChange && (
        <div className="flex items-center gap-1 p-1 rounded-lg bg-v6-surface-secondary">
          {(["line", "area", "bar"] as const).map((t) => (
            <motion.button
              ...
              className={cn(
                ...
                type === t
                  ? "bg-white text-v6-primary shadow-sm"
                  : "text-v6-text-muted hover:text-v6-text-secondary"
              )}
            >
...
function ChartSkeletonV7({ height }: { height: number }) {
  return (
    <div className="animate-pulse" style={{ height }}>
      <div className="h-full w-full rounded-xl bg-v6-surface-tertiary relative overflow-hidden">
        <div className="absolute inset-0 flex items-end justify-around px-4 pb-8">
          {[...Array(7)].map((_, i) => (
            <div
              key={i}
              className="w-8 bg-v6-surface-primary rounded-t"
  ...
  if (loading) {
    return (
      <motion.div
        ...
        className={cn("p-5 rounded-2xl bg-v6-surface-primary border border-v6-border", className)}
      >
        <ChartHeaderV7
          title={title}
          type={chartType}
        ...
        <ResponsiveContainer width="100%" height="100%">
          {chartType === "bar" ? (
            <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              />
              <Tooltip
                content={<CustomTooltipV7 format={format} />}
            ...
            <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <YAxis
                ...
                width={60}
              />
              <Tooltip content={<CustomTooltipV7 format={format} />} />
              <ReferenceLine
                y={avgValue}
                stroke={V7_COLORS.textMuted}
                strokeDasharray="4 4"
...
export interface SparklineV7Props {
  data: number[];
  color?: "primary" | "secondary" | "green" | "error";
  width?: number;
  height?: number;
