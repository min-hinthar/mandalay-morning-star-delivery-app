...
export interface MenuItemCardV7Props {
  /** Menu item data */
  item: MenuItem;
...
function DietaryBadge({ type, level = 1 }: DietaryBadgeProps) {
  const badges = {
    vegetarian: {
      ...
      label: "Vegetarian",
      color: "bg-v6-green/20 text-v6-green border-v6-green/30",
    },
    vegan: {
      icon: <Leaf className="w-3 h-3" />,
      label: "Vegan",
    ...
    spicy: {
      ...
      color: "bg-v6-primary/20 text-v6-primary border-v6-primary/30",
    },
    popular: {
      icon: <Star className="w-3 h-3 fill-current" />,
      label: "Popular",
      color: "bg-v6-secondary/20 text-v6-secondary-hover border-v6-secondary/30",
    },
  };
...
export function MenuItemCardV7({
  item,
  onSelect,
  onQuickAdd,
  onFavoriteToggle,
  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden bg-v6-surface-secondary">
        <motion.div
        ...
        {item.isSoldOut && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-10">
            <span className="px-4 py-2 bg-v6-surface-primary rounded-full text-sm font-semibold text-v6-text-primary">
              Out of Stock
            </span>
          </div>
        )}
      ...
      <div className="p-4">
        ...
        <h3 className="font-v6-display font-semibold text-lg text-v6-text-primary mb-1 line-clamp-1">
          {item.nameEn}
        </h3>
        <p className="font-v6-body text-sm text-v6-text-secondary mb-3 line-clamp-2">
          {item.descriptionEn}
        </p>
      ...
      {shouldAnimate && (
        <motion.div
          className="absolute inset-0 rounded-v6-card pointer-events-none"
          style={{
            boxShadow: isHovered
