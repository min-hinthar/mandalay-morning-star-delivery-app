...
import { ModifierToggleV7 } from "./ModifierToggleV7";
import { VisualPreview } from "./VisualPreview";
import { MenuItemCardV7 } from "./MenuItemCardV7";
...
export interface ItemDetailV7Props {
    ...
    <div className="flex items-center gap-3">
      <motion.button
        ...
        className={cn(
          ...
          "bg-v6-surface-secondary text-v6-text-primary",
      ...
      <AnimatePresence mode="wait">
        <motion.span
          key={value}
          className="w-12 text-center font-v6-display text-2xl font-bold text-v6-text-primary"
...
function HeroImage({ src, alt, scrollProgress }: HeroImageProps) {
  ...
  return (
    <div className="relative h-72 md:h-96 overflow-hidden bg-v6-surface-secondary">
      <motion.div
      ...
      {/* Gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-t from-v6-surface-primary via-transparent to-transparent" />
    </div>
  );
}
...
function RelatedItems({ items, onSelect }: RelatedItemsProps) {
  ...
  return (
    <motion.section
      className="mt-8 pt-8 border-t border-v6-border"
      initial={shouldAnimate ? { opacity: 0, y: 20 } : undefined}
      animate={shouldAnimate ? { opacity: 1, y: 0 } : undefined}
      transition={{ ...getSpring(v7Spring.default), delay: 0.3 }}
    >
      <h3 className="font-v6-display font-semibold text-lg text-v6-text-primary mb-4">
        You Might Also Like
      </h3>
      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4">
        {items.map((item, index) => (
          <motion.div
            ...
            transition={{ ...getSpring(v7Spring.snappy), delay: index * 0.1 }}
          >
            <MenuItemCardV7
              item={item}
              onSelect={onSelect}
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          ...
          "bg-v6-surface-primary rounded-t-3xl md:rounded-3xl",
      ...
      >
        ...
        <div className="absolute top-4 left-4 right-4 z-20 flex items-center justify-between">
          <motion.button
            ...
            aria-label="Close"
          >
            <ChevronLeft className="w-5 h-5 text-v6-text-primary" />
          </motion.button>
          ...
          <div className="flex items-center gap-2">
            {onFavoriteToggle && (
              ...
              >
                <Heart
                  className={cn(
                    "w-5 h-5",
                    isFavorite ? "fill-v6-primary text-v6-primary" : "text-v6-text-primary"
                  )}
                />
              </motion.button>
            )}
            <motion.button
              ...
              aria-label="Share"
            >
              <Share2 className="w-5 h-5 text-v6-text-primary" />
            </motion.button>
          </div>
        </div>
        ...
        >
          ...
          <div className="px-6 py-6 pb-32">
            ...
            <motion.div
              initial={shouldAnimate ? { opacity: 0, y: 20 } : undefined}
              animate={shouldAnimate ? { opacity: 1, y: 0 } : undefined}
              transition={{ ...getSpring(v7Spring.default), delay: 0.1 }}
            >
              <h2 className="font-v6-display text-2xl font-bold text-v6-text-primary mb-2">
                {item.nameEn}
              </h2>
              <p className="font-v6-body text-v6-text-secondary mb-4">
                {item.descriptionEn}
              </p>
            </motion.div>
            ...
            {/* Quantity selector */}
            <motion.div
              className="flex items-center justify-between py-4 border-t border-v6-border"
              initial={shouldAnimate ? { opacity: 0, y: 20 } : undefined}
              animate={shouldAnimate ? { opacity: 1, y: 0 } : undefined}
              transition={{ ...getSpring(v7Spring.default), delay: 0.2 }}
            >
              <span className="font-v6-body font-medium text-v6-text-primary">
                Quantity
              </span>
        ...
        {/* Fixed bottom bar */}
        <motion.div
          className="absolute bottom-0 left-0 right-0 p-4 bg-v6-surface-primary border-t border-v6-border"
          initial={shouldAnimate ? { y: 100 } : undefined}
          animate={shouldAnimate ? { y: 0 } : undefined}
          transition={{ ...getSpring(v7Spring.snappy), delay: 0.3 }}
        >
          <div className="flex items-center gap-4">
            {/* Total price */}
            <div className="flex-1">
              <span className="text-sm text-v6-text-muted font-v6-body">Total</span>
              <div className="font-v6-display text-2xl font-bold text-v6-primary">
            ...
            <motion.button
              ...
              className={cn(
                ...
                "font-v6-body font-semibold text-white",
                canAddToCart && !item.isSoldOut
                  ? "bg-v6-primary shadow-lg shadow-v6-primary/30"
                  : "bg-v6-text-muted cursor-not-allowed"
              )}
              whileHover={
