...
import { CategoryCarouselV7 } from "./CategoryCarouselV7";
import { MenuItemCardV7 } from "./MenuItemCardV7";
import { ItemDetailV7 } from "./ItemDetailV7";
...
export interface MenuLayoutV7Props {
  ...
  return (
    <motion.div
      className={cn(
        ...
        "bg-v6-surface-secondary border-2 transition-colors",
        isFocused ? "border-v6-primary" : "border-transparent"
      )}
      animate={
    ...
    >
      <motion.div
        ...
        transition={getSpring(v7Spring.snappy)}
      >
        <Search className="w-5 h-5 text-v6-text-muted" />
      </motion.div>
      ...
      <input
        ...
        className={cn(
          ...
          "font-v6-body text-v6-text-primary placeholder:text-v6-text-muted"
        )}
      />
      ...
      <AnimatePresence>
        {value && (
          <motion.button
            type="button"
            onClick={handleClear}
            className="p-1 rounded-full hover:bg-v6-surface-primary"
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {activeCount > 0 && (
          <motion.span
            className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-v6-primary text-white text-xs flex items-center justify-center font-bold"
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        variants={shouldAnimate ? v7StaggerContainer() : undefined}
        initial={shouldAnimate ? "hidden" : undefined}
        whileInView={shouldAnimate ? "visible" : undefined}
        viewport={{ once: true, margin: "-50px" }}
      >
        {items.map((item, index) => (
          <motion.div key={item.id} variants={shouldAnimate ? v7StaggerItem : undefined}>
            <MenuItemCardV7
              item={item}
...
function EmptyState({ query }: EmptyStateProps) {
  ...
  return (
    ...
    >
      ...
      <h3 className="font-v6-display text-xl font-semibold text-v6-text-primary mb-2">
        No items found
      </h3>
      <p className="font-v6-body text-v6-text-secondary max-w-sm">
        {query
...
export function MenuLayoutV7({
  ...
  return (
    <LayoutGroup>
      <div className={cn("min-h-screen bg-v6-surface-primary", className)}>
        {/* Search and filter bar */}
        <motion.div
          className="sticky top-0 z-20 bg-v6-surface-primary/95 backdrop-blur-lg border-b border-v6-border"
        ...
        <main className="max-w-7xl mx-auto px-4 py-6">
          ...
          <AnimatePresence mode="wait">
            {localSearchQuery && (
              <motion.p
                className="text-sm text-v6-text-muted mb-4 font-v6-body"
