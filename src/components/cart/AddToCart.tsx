...
function ConfettiParticle({ index, color, origin }: ConfettiParticleProps) {
...
function FlyingItem({ imageUrl, startPos, endPos, onComplete }: FlyingItemProps) {
  ...
  return (
    ...
    >
      <div className="w-16 h-16 rounded-full overflow-hidden shadow-xl bg-surface-secondary">
        {imageUrl ? (
...
export function CartBadge({ count, shouldBounce }: CartBadgeProps) {
...
export interface MiniAddToCartProps {
  item: MenuItem;
  onAdd?: () => void;
  className?: string;
}

export function MiniAddToCart({ item, onAdd, className }: MiniAddToCartProps) {
  const { shouldAnimate, getSpring } = useAnimationPreference();
  const { addItem } = useCart();
  const { open: openDrawer } = useCartDrawer();
