...
function StatItem({ icon, label, value, delay = 0 }: StatItemProps) {
  ...
  return (
    <motion.div
      ...
      transition={shouldAnimate ? { ...getSpring(v7Spring.default), delay } : undefined}
    >
    ...
    >
      ...
      <ParallaxContainer height="100svh" className="absolute inset-0">
        ...
        <ParallaxLayer speed="mid" zIndex={2}>
          <div className="absolute inset-0 bg-gradient-radial from-v6-secondary/20 via-transparent to-transparent" />
        </ParallaxLayer>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          {showMascot && isMounted && (
            <motion.div
              ...
              transition={shouldAnimate ? { ...getSpring(v7Spring.rubbery), delay: 0.2 } : undefined}
            >
              <BrandMascot
                size="lg"
                expression="waving"
          ...
          <motion.div
            ...
            initial={shouldAnimate ? { opacity: 0, y: -20 } : undefined}
            animate={shouldAnimate ? { opacity: 1, y: 0 } : undefined}
            transition={shouldAnimate ? { ...getSpring(v7Spring.default), delay: 0.3 } : undefined}
          >
            <span className="text-v6-secondary">
              {timeOfDay === "morning" && "🌅"}
          ...
          <AnimatedHeadline
            text={headline}
            className="font-v6-display text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-white mb-6 leading-tight"
          />

          {/* Subheadline */}
          <motion.p
            className="text-lg md:text-xl text-white/80 max-w-2xl mx-auto mb-10 font-v6-body"
            initial={shouldAnimate ? { opacity: 0, y: 20 } : undefined}
            animate={shouldAnimate ? { opacity: 1, y: 0 } : undefined}
            transition={shouldAnimate ? { ...getSpring(v7Spring.default), delay: 0.6 } : undefined}
          >
            {subheadline}
          </motion.p>
          ...
          <motion.div
            ...
            initial={shouldAnimate ? { opacity: 0, y: 20 } : undefined}
            animate={shouldAnimate ? { opacity: 1, y: 0 } : undefined}
            transition={shouldAnimate ? { ...getSpring(v7Spring.default), delay: 0.8 } : undefined}
          >
            <Link href={ctaHref}>
              <motion.div
              ...
              >
                <Button
                  variant="primary"
                  size="lg"
                  className="relative overflow-hidden group px-8 py-6 text-lg bg-v6-secondary hover:bg-v6-secondary-hover shadow-lg shadow-v6-secondary/30"
                >
          ...
          <motion.div
            ...
            initial={shouldAnimate ? { opacity: 0, y: 20 } : undefined}
            animate={shouldAnimate ? { opacity: 1, y: 0 } : undefined}
            transition={shouldAnimate ? { ...getSpring(v7Spring.default), delay: 1 } : undefined}
          >
            <StatItem
              icon={<ChefHat className="w-4 h-4 text-v6-secondary" />}
              label="Authentic"
            ...
            <StatItem
              icon={<Clock className="w-4 h-4 text-v6-secondary" />}
              label="Delivery"
            ...
            <div className="hidden md:block w-px h-10 bg-white/20" />
            <StatItem
              icon={<MapPin className="w-4 h-4 text-v6-secondary" />}
              label="Coverage"
      ...
      <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-v6-surface-primary to-transparent z-40 pointer-events-none" />
    </section>
  );
}
