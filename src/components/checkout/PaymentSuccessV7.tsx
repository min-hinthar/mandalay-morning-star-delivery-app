...
export interface PaymentSuccessV7Props {
  /** Order ID */
  orderId: string;
...
function ConfettiParticleV7({ color, delay }: Omit<ConfettiParticleV7Props, "index">) {
...
function TimelineStep({ icon, title, description, isActive, index }: TimelineStepProps) {
  ...
  return (
    <motion.div
      ...
      transition={{ ...getSpring(v7Spring.default), delay: 0.8 + index * 0.15 }}
    ...
    >
      ...
      <div>
        <p className="font-semibold text-v6-text-primary">{title}</p>
...
export function PaymentSuccessV7({
  orderId,
  totalCents,
  deliveryTime,
  deliveryAddress,
    ...
    <div className={cn("relative min-h-[80vh] flex flex-col", className)}>
      ...
      >
        ...
        <motion.div
          ...
          className="mt-6"
        >
          <h1 className="text-2xl sm:text-3xl font-v6-display font-bold text-v6-text-primary">
            Order Confirmed!
          </h1>
          <motion.div
          ...
          >
            ...
            <p className="text-v6-text-secondary">
        ...
        >
          <span className="text-sm text-v6-text-muted">Order ID:</span>
          <code className="px-3 py-1 rounded-lg bg-v6-surface-secondary font-mono text-sm text-v6-text-primary">
            {orderId}
          </code>
          <motion.button
            type="button"
            ...
            className="p-1.5 rounded-md hover:bg-v6-surface-tertiary transition-colors"
          >
            <AnimatePresence mode="wait">
              {copied ? (
                <motion.div
                  ...
                  exit={{ scale: 0 }}
                >
                  <Check className="w-4 h-4 text-v6-green" />
                </motion.div>
              ) : (
                <motion.div
                  key="copy"
                  ...
                  exit={{ scale: 0 }}
                >
                  <Copy className="w-4 h-4 text-v6-text-muted" />
                </motion.div>
              )}
            </AnimatePresence>
          </motion.button>
        ...
        >
          {/* Header */}
          <div className="p-4 bg-v6-surface-secondary border-b border-v6-border">
            <div className="flex items-center justify-between">
              ...
              <PriceTicker
                value={totalCents}
                className="text-xl font-bold text-v6-primary"
              />
            </div>
          </div>
          ...
          <div className="p-4 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-left">
                <p className="text-sm text-v6-text-muted">Delivery to</p>
                <p className="font-medium text-v6-text-primary">{deliveryAddress}</p>
              </div>
            </div>

            <div className="flex items-start gap-3">
              <Clock className="w-5 h-5 text-v6-primary flex-shrink-0 mt-0.5" />
              <div className="text-left">
                <p className="text-sm text-v6-text-muted">Estimated delivery</p>
                <p className="font-medium text-v6-text-primary">{deliveryTime}</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        {/* Timeline */}
        <motion.div
          variants={shouldAnimate ? v7StaggerContainer(0.15, 0.8) : undefined}
          initial="hidden"
          animate="visible"
...
export default PaymentSuccessV7;
