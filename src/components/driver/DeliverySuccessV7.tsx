...
/**
 * V7 Delivery Success - Motion-First Celebration Design
 *
...
export interface DeliverySuccessV7Props {
  /** Stop details */
  stop: {
    id: string;
    customerName: string;
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="absolute inset-0 rounded-full bg-v6-green/30"
          ...
          {[...Array(6)].map((_, i) => {
            ...
            return (
              <motion.div
                ...
                className="absolute top-1/2 left-1/2 w-2 h-2 rounded-full bg-v6-secondary"
...
interface BadgeUnlockV7Props {
  badge: NonNullable<DeliverySuccessV7Props["unlockedBadge"]>;
}

function BadgeUnlockV7({ badge }: BadgeUnlockV7Props) {
  const { shouldAnimate, getSpring } = useAnimationPreferenceV7();
  ...
  return (
    <motion.div
      ...
      transition={{ ...getSpring(v7Spring.ultraBouncy), delay: 0.8 }}
      className={cn(
        "rounded-2xl overflow-hidden",
        "bg-gradient-to-r from-v6-secondary/20 via-v6-primary/20 to-v6-secondary/20",
        "border border-v6-secondary/30 p-4"
      )}
    >
      <div className="flex items-center gap-4">
        ...
        <motion.div
          ...
          className={cn(
            ...
            "bg-gradient-to-br from-v6-secondary to-v6-primary",
            "flex items-center justify-center text-3xl",
            "shadow-lg shadow-v6-secondary/30"
          )}
        >
          {badge.icon}
        </motion.div>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <Award className="w-4 h-4 text-v6-secondary" />
            <span className="text-xs font-semibold text-v6-secondary uppercase tracking-wider">
              Badge Unlocked!
            </span>
          </div>
          <p className="font-bold text-v6-text-primary">{badge.name}</p>
          <p className="text-sm text-v6-text-muted">{badge.description}</p>
        </div>
...
export function DeliverySuccessV7({
  stop,
  currentStop,
  totalStops,
  deliveryTimeMinutes,
    ...
    <div className={cn("relative min-h-[80vh] flex flex-col", className)}>
      ...
      >
        ...
        <motion.div
          ...
          className="mt-6"
        >
          <h1 className="text-2xl sm:text-3xl font-bold text-v6-text-primary">
            Delivery Complete!
          </h1>
          <motion.div
          ...
          >
            <PartyPopper className="w-5 h-5 text-v6-secondary" />
            <p className="text-v6-text-secondary">Great job!</p>
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          className={cn(
            ...
            "border border-v6-border shadow-v6-card",
            "overflow-hidden"
          )}
        >
          {/* Photo preview */}
          {stop.deliveryPhotoUrl && (
            <div className="relative h-32 bg-v6-surface-tertiary">
          ...
          <div className="p-4 space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="text-left">
                <p className="font-medium text-v6-text-primary">{stop.customerName}</p>
                <p className="text-sm text-v6-text-muted">{stop.address}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6 text-sm">
              ...
              <div className="flex items-center gap-1.5 text-v6-text-muted">
                <Trophy className="w-4 h-4 text-v6-secondary" />
                <span>
        ...
        {unlockedBadge && (
          <div className="mt-4 w-full max-w-md">
            <BadgeUnlockV7 badge={unlockedBadge} />
          </div>
        )}
        ...
        >
          <p className="text-sm text-v6-text-muted mb-3">
              ...
              >
                <Star
                  className={cn(
                    ...
                    star <= rating
                      ? "text-v6-secondary fill-v6-secondary"
                      : "text-v6-border"
                  )}
                />
              </motion.button>
        ...
        >
          ...
          {isLastStop && (
            <p className="mt-3 text-sm text-v6-text-muted">
...
export default DeliverySuccessV7;
