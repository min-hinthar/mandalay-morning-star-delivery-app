...
export const AttentionSection = memo(
  forwardRef<HTMLDivElement, AttentionSectionProps>(function AttentionSection(
    { items, validations, suggestions, onRemoveItem, onReplaceItem, className },
    ref
  ) {
    ...
    const problemItems = items.filter((item) => validations.has(item.cartItemId));
    ...
    return (
      <m.div
        ...
        exit={shouldAnimate ? { opacity: 0, height: 0, marginBottom: 0 } : undefined}
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-text-primary">
            {problemItems.length} {problemItems.length === 1 ? "item needs" : "items need"}{" "}
            attention
          </h3>
        </div>
