...
/** Maps KPI icon keys to their detail page */
const linkMap: Record<string, string> = {
  orders: "/admin/orders",
  revenue: "/admin/orders",
  drivers: "/admin/drivers",
  exceptions: "/admin/routes",
  target: "/admin/routes",
  activity: "/admin/orders",
};

const variantStyles = {
  default: {
    bg: "bg-gradient-to-br from-accent-teal/5 to-accent-teal/10",
    border: "border-accent-teal/20",
    iconBg: "bg-accent-teal/10",
    iconColor: "text-accent-teal",
  },
  success: {
    bg: "bg-gradient-to-br from-green/5 to-accent-teal/10",
    border: "border-green/20",
    iconBg: "bg-green/10",
    iconColor: "text-green",
  },
  warning: {
    bg: "bg-gradient-to-br from-secondary/5 to-accent-teal/5",
    border: "border-secondary/20",
    iconBg: "bg-secondary/10",
    iconColor: "text-secondary",
  },
  danger: {
    bg: "bg-gradient-to-br from-status-error/5 to-accent-teal/5",
...
export function KPICard({ data, index, refreshing, onGoalReached }: KPICardProps) {
  ...
  const href = linkMap[data.icon] || "/admin";
      ...
      >
          ...
          <div className="space-y-1.5">
            ...
            <div className="h-2 rounded-full bg-surface-tertiary overflow-hidden">
              <m.div
                ...
                className={cn(
                  "h-full rounded-full",
                  goalProgress >= 100 ? "bg-green" : "bg-accent-teal"
                )}
              />
            </div>
          </div>
        ...
        <AnimatePresence>
          {hasReachedGoal && goalProgress >= 100 && (
            ...
            >
              <div className="p-1.5 rounded-full bg-accent-teal shadow-lg">
