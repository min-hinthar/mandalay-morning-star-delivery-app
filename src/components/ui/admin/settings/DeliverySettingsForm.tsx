...
export function DeliverySettingsForm({
  ...
  const updateTimeWindow = useCallback(
    (index: number, updates: Partial<DeliveryTimeWindow>) => {
      const windows = settings.deliveryTimeWindows.map((w, i) =>
        i === index ? { ...w, ...updates } : w
      );
  ...
  const removeTimeWindow = useCallback(
    (index: number) => {
      onChange({
        ...settings,
        deliveryTimeWindows: settings.deliveryTimeWindows.filter((_, i) => i !== index),
      });
    },
    [settings, onChange]
  );
      ...
      <div className={cn("space-y-4", changed("deliveryTimeWindows") && CHANGED_BORDER)}>
        <div className="pb-3 border-b border-border-subtle">
          ...
          <p className="mt-1 text-xs text-text-secondary">
            Define available delivery time slots for customers.
          </p>
        </div>
          ...
          {settings.deliveryTimeWindows.map((window, index) => (
            ...
            >
              <div className="space-y-1">
                <Label htmlFor={`tw-start-${index}`} className="text-xs">
                  Start
                </Label>
                <Input
                  id={`tw-start-${index}`}
              ...
              <div className="space-y-1">
                <Label htmlFor={`tw-end-${index}`} className="text-xs">
                  End
                </Label>
                <Input
                  id={`tw-end-${index}`}
              ...
              <div className="space-y-1 flex-1 min-w-[120px]">
                <Label htmlFor={`tw-label-${index}`} className="text-xs">
                  Label (optional)
                </Label>
                <Input
                  id={`tw-label-${index}`}
