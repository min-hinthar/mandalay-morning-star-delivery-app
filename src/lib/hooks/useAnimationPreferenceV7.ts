 ...
 */
export type AnimationPreferenceV7 = "full" | "reduced" | "none";

const STORAGE_KEY = "v7-animation-preference";
const DATA_ATTRIBUTE = "data-v7-motion";
 ...
 *   shouldAnimate,
 *   getSpring,
 * } = useAnimationPreferenceV7();
 */
export function useAnimationPreferenceV7() {
  const [preference, setPreferenceState] = useState<AnimationPreferenceV7>("full");
  const [isHydrated, setIsHydrated] = useState(false);

  // Initialize on mount
  useEffect(() => {
    const stored = localStorage.getItem(STORAGE_KEY) as AnimationPreferenceV7 | null;
    const validStored =
   ...
   */
  const setPreference = useCallback((newPref: AnimationPreferenceV7) => {
  ...
  const cyclePreference = useCallback(() => {
    const cycle: AnimationPreferenceV7[] = ["full", "reduced", "none"];
 ...
 */
export function getAnimationPreferenceV7(): AnimationPreferenceV7 {
 ...
 */
export function shouldAnimateV7(): boolean {
  return getAnimationPreferenceV7() !== "none";
}

/**
 * Check if full motion is enabled (one-time)
 */
export function isFullMotionV7(): boolean {
  return getAnimationPreferenceV7() === "full";
}
 ...
 * @example
 * // In CSS/Tailwind
 * [data-v7-motion="none"] .animate-class { animation: none; }
 * [data-v7-motion="reduced"] .animate-class { animation-duration: 0.01s; }
 */
export const v7MotionDataAttribute = DATA_ATTRIBUTE;

/**
 * CSS selector helpers
 */
export const v7MotionSelectors = {
