...
- **Materialized views have NO implicit grants — `driver_stats_mv`/`delivery_metrics_mv` must be read via their `SECURITY DEFINER` wrappers** (`get_driver_stats_admin()` / `get_delivery_metrics_admin()`: granted to `authenticated`, self-gate on `is_admin()`, `RETURNS SETOF` so `.eq/.gte/.order` chain). A direct `.from("<mv>")` fails at runtime for EVERY caller and is invisible to tsc, db-drift, and the phantom-column guard — it shipped the admin analytics pages broken (empty panels + 500s). `materialized-view-access.test.ts` bans direct reads repo-wide. The views are created UNQUALIFIED in the baseline (grep bare, not `public.<mv>`).
