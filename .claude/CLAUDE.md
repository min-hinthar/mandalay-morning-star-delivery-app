...
Cloud-container note: bare `pnpm test` can OOM here — the full suite RUNS via
`node --max-old-space-size=3072 ./node_modules/vitest/vitest.mjs run --pool=forks --maxWorkers=2`
(~1680 tests, ~75s). Capture the exit code directly; `cmd; echo $?; tail log` reports tail's status.
(`--poolOptions.*` is not a valid vitest 4 flag — it aborts.)
...
- **A `.returns<T>()` cast can hide a column that does not exist, and NOTHING catches it** — not `tsc` (the cast replaces the generated type), not `db-drift` (it compares schema to types; neither knows what the app SELECTs). PostgREST rejects the WHOLE query for one unknown column (42703 / HTTP 400), so the failure is total, not partial. `src/lib/__tests__/select-columns-exist.test.ts` is the guard; it validates top-level select columns, columns nested inside embedded relation bodies (against the RELATION's table), and filter columns, with floors so a deleted query cannot quietly shrink coverage.
- **"Cancelled" has no single home, and reading the newest audit row is only safe if you never SKIP one.** `orders` has no `cancelled_at`/`cancellation_reason` column. A cancellation is recorded in FOUR places: `order_audit_log` `action='cancel'` (admin cancel route), `order_audit_log` `action='status_change'` with `new_value.status='cancelled'` (admin status route — easy to miss, and `approve-cod` plus every forward transition share that action), a note on `orders.special_instructions` (account self-serve), and nowhere at all (pending-order route, Stripe `charge-refunded`, `checkout-session-expired`). Because `cancelled -> pending` is a permitted transition and FIVE of those paths write no audit row, scanning back past a non-cancelling row for an older `cancel` row attributes a superseded reason — and its stale timestamp — to an unrelated cancellation. Read the NEWEST `cancel|status_change` row and stop: both kinds are decisive, so an older row is by definition superseded.
- **A swallowed read error must not reach the client as an authoritative null.** `getOrderCancellation` catches a failed audit-log read and returns no cancellation, while the API still answers 200 — so a client that treats "fetch succeeded" as "answer is authoritative" lets a transient failure overwrite real data with nothing and keep it hidden. Return the OUTCOME (`{ ok, cancellation }` → `order.cancellationKnown`), and adopt values only when the read actually succeeded. Same rule as "a failure must never read as empty", one process boundary out.
- **A ref latched in an effect's cleanup must be re-armed at setup.** Cleanup runs on every dependency change — and twice on mount under React Strict Mode (`next.config` enables it) — not only on unmount. A cleanup-only `abortRef.current = true` is therefore permanently true from the first dep change onward, silently killing whatever it guards (in `useTrackingSubscription`, the entire cancellation-detail retry). Unmount→remount does NOT reproduce it: a fresh hook gets a fresh ref. Also settle any promise you abandon — `clearTimeout` on an awaited `new Promise(resolve => setTimeout(resolve, d))` leaves the loop hung forever instead of returning; store the resolver and call it after latching the abort flag.
- **Populating a field that was always null can wake a dead code branch.** `StatusStepper` computes `currentStatus === "cancelled" || !!cancelledAt`; `cancelledAt` was hardcoded null repo-wide, so that `||` had never once fired. Filling the field in gave a FROZEN SSR snapshot a way to disagree with live status — the rail stayed cancelled after `cancelled -> pending` while the overlay and tab title said pending. Before populating a previously-dead field, grep every consumer for what it does with a truthy value.

- **Stripe promo-code enforcement is Stripe-path ONLY — COD bypasses all of it.** `max_redemptions`, `first_time_transaction` and `times_redeemed` only see Stripe Checkout sessions, so a one-time code (every `KYAYZU-` loyalty code is `max_redemptions:1`) was reusable indefinitely via cash on delivery. `resolveCheckoutDiscount` now takes `paymentMethod` and adds COD orders to the redemption count, checks `loyalty_rewards.redeemed_at`, and runs the prior-order gate for every first-time code. **For anything that must be single-use, prefer app coupons** (`coupons` table, admin → Coupons): `claim_coupon` (service_role-only RPC, `FOR UPDATE`) binds the coupon to the order AFTER the order row exists; a losing claim deletes that order (409). A coupon is consumed while `order_id` points at a live order — it frees itself only when the holder is cancelled/deleted (FK `ON DELETE SET NULL`) or an UNPAID card checkout (`payment_method='stripe'`, no PI) idle >2h since the LAST claim (`redeemed_at`) — a paid or COD order an admin reverts to `pending` keeps its hold. The current holder always re-claims (even after expiry/revoke); retry-payment and approve-cod call `reconfirmCouponHold` so a payable session never outlives its hold and an orphaned duplicate can't be approved. Limited Stripe codes: redemptions = max(Stripe tally, paid card orders) + COD + others' open checkouts (retry-payment charges via a one-off coupon Stripe doesn't tie to the code); `KYAYZU-` codes also have a partial unique index on live orders for the concurrent case. Stripe coupon `name` is capped at 40 chars — retry-payment's old `Order #<uuid> discount` (52) broke every discounted retry. Free delivery is a **fee waiver** (`applyDeliveryWaiver` lowers `delivery_fee_cents`), never `discount_cents` — discount feeds loyalty net spend and the proportional item-refund math. `src/lib/coupons/index.ts` imports `node:crypto`: client code imports only `@/lib/coupons/effect` / `/status`
- **`orders` has no customer DELETE policy — deleting through the user-scoped client is a silent 0-row no-op.** The old `cleanupOrder` (revalidation failure) left `pending`, session-less orders that `reclaimPendingCheckouts` can never clear (it requires a session id), permanently withholding the first-order discount. Discard never-paid orders with the SERVICE client, and discard on ANY throw before the Stripe session exists (`stripe-session.ts`)
...
- **A same-origin redirect guard needs more than `startsWith("/") && !startsWith("//") && !includes("://")`** — the WHATWG URL parser normalizes before resolving, so `/\evil.com` (backslash aliases to `/` for http/https) and `/<TAB>/evil.com` (tab, LF and CR are STRIPPED pre-parse) both resolve to `https://evil.com`. That predicate had been copy-pasted into FOUR files; it now lives once in `src/lib/utils/safe-redirect.ts`, rejects `[\\\t\n\r]`, and finally resolves the candidate against a throwaway origin and demands the same origin back (the belt that catches the next normalization quirk). Matters most where `?next=` is on the normal path — the middleware attaches it to every unauthenticated /checkout·/cart·/orders·/account hit
- **A "gate closed" flag is not the same as "cannot order" — never fold one straight into a submit gate.** The LEGACY delivery gate's `isOpen` means "THIS Saturday's cutoff passed": at Fri 4pm PT `computeDeliveryGate(5,15).isOpen` is `false` while `getAvailableDeliveryDates` still returns the next two Saturdays with `cutoffPassed:false` — dates the picker offers and the server accepts. Folding a bare `!gate.isOpen` into `cutoffModalOpen` disabled Place Order for that whole window with no escape (the modal's reschedule button needs multi-day config to render). Gate on the SELECTED date instead (`delivery.date < gate.deliveryDate.dateString`). Multi-day differs: `computeDeliveryGateMultiDay` rolls to the next open run, so its `isOpen:false` really does mean "no active run exists" and blocking IS right. **Over-blocking is as bad as under-blocking** — when a review asks you to tighten a gate, check what the tightened gate does to the VALID case, and write the test for that case too
- **Cross-store cleanup on `reset()` is a trap when the reset fires on unmount.** `CheckoutClient` calls `checkout-store.reset()` on every non-Stripe unmount — i.e. simply navigating /checkout → /menu — so having `reset()` null the CART store's `addressDistanceMiles` dropped a far-address customer to LOCAL pricing for the rest of the session: the free-delivery meter promised FREE while the server would still charge the extended fee, and the drawer's minimum gate re-enabled the cart checkout had just blocked. Keeping the last known distance is the conservative direction (over-quotes, never baits), self-corrects on the next `setAddress`, and isn't persisted (cart `partialize` keeps only `items`). The real fix is to derive the distance from the resolved default address globally, not to clear it
- **RLS on `drivers`/`profiles` is ASYMMETRIC — admins can SELECT/INSERT/DELETE but NOT UPDATE.** Verified in `supabase/migrations/00000000000000_baseline.sql`: `drivers_select`/`profiles_select` are `(owner OR is_admin())`, `drivers_insert` is `WITH CHECK (is_admin())`, `drivers_delete` is `USING (is_admin())` — but `drivers_update` and `profiles_update` are **owner-only** (`user_id/id = auth.uid()`) with a matching `WITH CHECK`, no `is_admin()` clause. So every admin UPDATE through the caller-scoped client matched ZERO rows, and because `.update()` returns no row count (see the gotcha below) a zero-row update carries no error — four endpoints returned 200 while changing nothing (driver field edit, DELETE-as-soft-delete, archive, and the profile promotion in driver-create, which stranded accounts that appear in the driver picker but can't sign in). Fix pattern: authenticate with the USER client, then write with `createServiceClient()` **after** the admin gate (authz proven before elevation), scoped by an explicit `.eq("id", …)`, and verify with `.select("id")`. Note DELETE-shaped endpoints that soft-delete via an UPDATE hit `drivers_update`, not `drivers_delete`, so `is_admin()` on the delete policy doesn't save them. Widening the two policies with `is_admin()` looks like the tidier fix and is **deliberately NOT done**: `profiles_update` with `is_admin()` would let any admin JWT PATCH profiles **directly through PostgREST**, bypassing the app-level invariants that live in the route — the admin-demotion guard (`profiles.role` is a single enum, so "promote to driver" also strips admin), the Zod validation, and the rate limit. That would reopen the hole via a second unguarded path. The service-client pattern is not a workaround for missing RLS here; it is the better shape, because it keeps writes on the one path that enforces those invariants. Leave the asymmetry alone
...
- **CSP `'unsafe-eval'` CANNOT be dropped or dev-gated — this has now been re-tried and re-reverted once already.** Google Maps JS API requires it in BOTH its allowlist AND its nonce/`strict-dynamic` CSP configurations (doc re-verified live 2026-08-05), the SDK rotates weekly server-side (a passing eval-free test today proves nothing about next week), and checkout's Places autocomplete rides the same SDK — regressing it is a revenue outage. History: dev-only gating shipped in `8bc23ed5` (Phase 67), reverted in `159cf8ee`; a 2026-08-05 session re-drafted the same gate and killed it during verification. Rationale is pinned in `next.config.ts` above `script-src` — read it before touching that directive. `connect.facebook.net` there is also deliberate (FB in-app browser injects its scripts; removing it re-floods Sentry with CSP violations, `dfd4f628`). The REAL D9 remainder is nonce-based removal of `'unsafe-inline'` (per-request CSP; still keeps eval per Google) — a dedicated project, not a config toggle.
...
- **"Confirmed" does NOT imply "paid" — gate advancing into fulfillment on payment (incident #71DC108A: a declined card order was confirmed-for-delivery with a null PI).** The paid predicate for a Stripe order is `stripe_payment_intent_id IS NOT NULL` (only the `checkout.session.completed` webhook + `verify-payment` write it, both after verifying real payment; a `session_<id>` placeholder counts as paid). The admin generic status route (`PATCH /api/admin/orders/[id]/status`) historically gated ONLY COD (`pending_approval→confirmed` → `/approve-cod`) and had NO "Stripe must be paid" gate, so an unpaid/declined `pending` card order could be one-click "Confirm Order"'d — and that route never sets a PI, so it minted a confirmed order with a null PI that the delivery pipeline (route-builder→driver→start, all status-only) would happily dispatch. Fix = reject `payment_method!='cod' && stripe_payment_intent_id IS NULL` into any fulfillment status (`confirmed`/`preparing`/`out_for_delivery`/`delivered`) at `/status` (root cause) + at route creation (defense) + disable the admin "Confirm Order" button + "Payment not received" badge for `awaiting_payment` orders. The durable belt is a DB trigger (`app_private.enforce_paid_before_fulfillment`, function in `app_private` so it doesn't drift `public` generated types) — drafted in `docs/gate-confirm-on-payment-plan.md`, deferred to a Docker session. NOTE: the Supabase MCP project attached to this workspace (`fasnpdhtvqtzjlvruqcu`, "MMS QR Platform", tables `qr_orders`) is a DIFFERENT app — the delivery DB (`ukuzkhuppqwtrdkjqrkv`) is NOT MCP-reachable, but the Stripe MCP IS the real "Mandalay Morning Star LLC" account (acct_1LpNEtD7LsBxOcnN) for read-only forensics (metadata search is exact-match only; `order_id` lives on the Checkout Session, not the PI).

- **Materialized views have NO RLS and DO inherit the platform default ACL — `driver_stats_mv`/`delivery_metrics_mv` are REVOKEd from anon/authenticated and must be read via their `SECURITY DEFINER` wrappers** (`get_driver_stats_admin()` / `get_delivery_metrics_admin()`: granted to `authenticated`, self-gate on `is_admin()`, `RETURNS SETOF` so `.eq/.gte/.order` chain). The old premise here ("no GRANT names them, so nobody can read them") was FALSE: Supabase's default privileges (outside the repo's migrations) grant anon+authenticated on every relation `postgres` creates, so locally even `anon` could `SELECT` every driver's email + fleet revenue. `20260925120000_privilege_hardening.sql` REVOKEs; **any migration that re-creates an `_mv` must repeat that REVOKE after the CREATE** (default privileges re-grant on create — `materialized-view-access.test.ts` enforces the ordering). Grepping migration text can never prove "no access" — check `has_table_privilege` (`supabase/tests/02_materialized_views.test.sql`). The views are created UNQUALIFIED in the baseline (grep bare, not `public.<mv>`).
- **Every `public` table is fully DML-granted to anon + authenticated by default ACL — row policies are the ONLY guard, and they don't restrict COLUMNS.** An owner-scoped `UPDATE` policy on `profiles` let any user `SET role='admin'` (→ `is_admin()` true, full admin). `20260925120000_privilege_hardening.sql` closes the class: column-level GRANTs (`profiles` INSERT `(id,email,role)` / UPDATE `(full_name,phone,updated_at)`; `drivers` UPDATE self-service cols only; `addresses` UPDATE `(is_default,updated_at)` — geocoded cols are pricing inputs, so address INSERT/PUT go through the service client after the ownership check), guard triggers in `app_private` (`guard_profile_role`, `guard_order_client_update` — owner may only cancel/edit notes/dismiss rating; route driver only `out_for_delivery`/`delivered`/`needs_contact`; `guard_route_stop_identity`), no client INSERT on `orders`/`order_items`/`order_item_modifiers` (checkout writes via service client), `profiles_insert_own` pins email to the JWT, anon reads `app_settings` via column grant `(key,value,category)` only, `get_my_driver_id()` requires `drivers.is_active` (so every driver policy + `order_on_my_route` drops a deactivated driver — mirrors `requireDriver`), and TRUNCATE/TRIGGER/REFERENCES revoked schema-wide + from default privileges. Rules: (a) column GRANTs FAIL CLOSED — a NEW column a client legitimately writes needs its own `GRANT UPDATE (col)` or the write 42501s; (b) the guard triggers key on `current_user IN ('anon','authenticated')` and exempt `is_admin()` — service-role and SECURITY DEFINER paths pass untouched, so move any new client write that needs more than that to a service-client route; (c) **permissive policies OR per CLAUSE, not per policy** — one policy's USING plus ANOTHER's WITH CHECK admits a row neither would alone (a driver whose own order is on their route passed USING via `orders_update_driver` and WITH CHECK via the customer-cancel policy → `delivered → cancelled` → cron auto-refund), so bound every transition in the guard trigger, never only in one policy's USING; (d) `supabase/tests/03_privilege_hardening.test.sql` pins it (pgTAP — not run in CI; run locally after touching grants).
- **RLS hides data SILENTLY — an embed the caller can't SELECT comes back `null`, and a write no policy admits matches 0 rows, both with NO error.** A full replay of every user-scoped read/write after #257 found a dozen pre-existing breakages of exactly that shape (#258): drivers got null addresses / customer profiles / order items (fix: `app_private.address_on_my_route` / `order_item_on_my_route` SELECT policies; take the customer's name/phone from the `orders.customer_name/customer_phone` checkout snapshot, never a `profiles` embed); customer tracking never saw the route or driver (fix: `loadRouteTracking` in `src/lib/tracking/route-tracking.ts` — service-client reads AFTER the owner-scoped order read); notes / rating-dismiss writes hit no customer UPDATE policy (fix: scoped service-client routes, not a wider policy). Rules: (a) a policy's subquery runs under the CALLER's RLS — `location_updates_select` joined `routes`, which customers can't see, so it never matched; wrap cross-table checks in a SECURITY DEFINER `app_private` helper; (b) a SECURITY INVOKER RPC that does `SELECT … FOR UPDATE` needs an UPDATE policy for the caller (`apply_item_refunds` failed for every admin: `order_items` had none); (c) `order_audit_log.actor_id` FKs `profiles.id` — drivers pass `userId`, not `driverId` — and `order_audit_log_action_check` must list every action the app writes (a new action needs a migration); (d) Supabase storage rejects `DELETE FROM storage.objects` ("Use the Storage API") — remove objects via `storage.from(bucket).remove()`, never from a trigger; (e) **fixing a dead read makes it LIVE — scope what it now returns to what the viewer needs NOW.** The first cut of the tracking fix handed every customer on a route the whole GPS trail (dwell clusters = earlier customers' homes) and the driver's personal phone on any order, forever; the fix scopes both to the customer's own leg (`app_private.location_visible_to_my_order`: own stop `enroute`/`arrived`, point `recorded_at >= rs.updated_at`, route `in_progress`, order `out_for_delivery`; `loadRouteTracking.onMyLeg` mirrors it) and drops driver address reads once a route is `completed`. A skipped stop leaves the order `out_for_delivery` — gate on the viewer's STOP, not just the order. When mutation-testing these policies, a single-policy mutation can stay green because another layer holds (a policy subquery on `routes` is itself driver-scoped; an UPDATE's new row is re-checked against SELECT policies) — mutate both layers before calling a test weak. When adding a user-scoped read or write, replay it as that role on the local stack (`BEGIN … SET LOCAL ROLE authenticated; set_config('request.jwt.claims', …) … ROLLBACK`) and treat null/0-row as a failure. `supabase/tests/04_rls_gap_fixes.test.sql` pins the fixes.
- **`drivers.rating_avg`'s unrated sentinel is 0, not null** (`DEFAULT 0`, trigger writes `COALESCE(v_new_avg, 0)`; `driver_ratings` CHECK `rating >= 1` makes 0 unambiguous — a rated driver can never average below 1). A `!== null` filter excludes nobody, so aggregates count unrated drivers as genuine 0-star scores (fleet average read "1.0" with one 4.8 driver). Use `computeFleetAverageRating`. And JSX `{value && <span/>}` renders a literal `0` — guard numbers with `value !== null`/`> 0`, never truthiness.
- **A guard that greps generated/dumped files must itself be falsified — five guard defects in one PR were all "green for the wrong reason"** (unbounded `$function$` slice matching a NEIGHBOURING function's `is_admin()` gate; `SetofOptions` keys absorbed as columns; `.rpc(`-blind chunking binding filters to the previous query; nested embeds resolved as functions; a grant check matching only grants that NAME the view). For each assertion: make the guarded fact false, confirm red, restore md5-identical. Full checklist: `.claude/learnings/testing.md`.

- **Status emails must be wired into the DRIVER flow, not just the admin `/status` route.** `out_for_delivery` and `delivered` also transition from driver actions (`/api/driver/routes/[routeId]/start` batches orders → `out_for_delivery`; `/stops/[stopId]` marks a stop `delivered`) AND the admin route-UI stop routes (`/api/admin/routes/[id]/stops[/[stopId]]`) — all of which previously did the status write but sent NO customer email, so a customer only got "on its way"/"delivered" if an admin drove it through the orders dashboard. One shared sender (`src/lib/email/order-status-email.ts` → `sendOrderStatusEmail(orderId, status, {driverName?, deliveredAt?})`) owns confirmed/out_for_delivery/delivered: it uses the SERVICE client (so it never depends on the caller's RLS scope — a driver can't read the customer's profile/address, an admin can) and a **STABLE idempotency key `<status>-<orderId>`** (NOT the old admin route's `status-<status>-<id>-<Date.now()>`, which defeats idempotency). Stable keys matter because (a) the driver start route is idempotent/retryable and the stop PATCH rides an at-least-once offline queue, and (b) `delivered` can fire from BOTH the driver stop route AND the admin route-UI — a stable key lets Resend dedupe multi-surface/retry sends into ONE email instead of double-notifying. Gate the send on the rows that ACTUALLY transitioned (`.update(...).in("status",[...]).select("id")` → non-empty), fire it via `after()` (never bare `void`), and swallow send errors (a notification must never fail the driver's delivery write). Wiring `after()` into a route means its unit tests must mock `next/server`'s `after` to run inline (see `lifecycle.test.ts`) — otherwise `after()` throws "outside a request scope" → 500. The web-**push** companion has the SAME reach gap and the SAME fix: `sendOrderStatusPush(orderId, status, {userId?})` (`src/lib/push/order-status-push.ts`) mirrors the email sender (service client, self-resolves the customer, no-op without VAPID) and is wired into all the same surfaces. Push needs NO idempotency key the way email does — its `order-<orderId>` coalescing tag makes a duplicate/successive fire replace the prior notification on the device. Fire email + push in SEPARATE try/catch blocks so one failing never skips the other.
