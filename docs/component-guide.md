...
export function ItemCard({ item, onSelect }: ItemCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="p-4">
        <h3 className="font-body font-semibold text-charcoal-900 line-clamp-1">{item.nameEn}</h3>
        {item.nameMy && (
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden p-0 bg-cream-50">
        ...
        <div className="relative h-48 sm:h-64 bg-charcoal-100">
          {item.imageUrl ? (
            <Image src={item.imageUrl} alt={item.nameEn} fill className="object-cover" priority />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
        ...
        <div className="overflow-y-auto max-h-[50vh] p-6">
          ...
          {/* Description */}
          {item.descriptionEn && (
            <p className="text-charcoal-600 mt-4 leading-relaxed">{item.descriptionEn}</p>
          )}
          ...
          <div className="mt-6">
            ...
            <Textarea
              ...
              rows={3}
            />
            <p className="text-xs text-charcoal-500 mt-1 text-right">{notes.length}/500</p>
          </div>
        </div>
...
export function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  const { items, clearCart, getItemsSubtotal, getEstimatedDeliveryFee, getItemCount } =
    useCartStore();
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="right" className="w-full sm:max-w-md bg-cream-50 flex flex-col p-0">
        {/* Header */}
        <SheetHeader className="p-6 border-b border-saffron-200">
          <div className="flex items-center justify-between">
            <div>
              <SheetTitle className="font-display text-2xl text-charcoal-900">Your Cart</SheetTitle>
            ...
            <SheetFooter className="p-6 border-t border-saffron-200 bg-cream-100 mt-auto">
              <div className="w-full space-y-4">
                ...
                <div className="border-t border-saffron-200 pt-4">
                  ...
                  <p className="text-xs text-charcoal-500 mt-1">Tax calculated at checkout</p>
                </div>
