...
| #243 | `claude/sw-authed-denylist`            | **D7:** SW never caches authed pages/API — all three halves (NavigationRoute, RSC soft-nav/prefetch, `apis` JSON) + activation purge                                                | CACHE_VERSION v5; only `/api/menu` keeps offline (explicit handler); boundary-anchored prefixes                                                                  |
| #244 | `claude/feedback-email-abuse`          | **D8:** feedback confirmations only to authed users' own email; `feedback-anon` rate tier (3/10m)                                                                                   |                                                                                                                                                                  |
| #245 | `claude/sentry-pii-fonts`              | **D10:** `sendDefaultPii:false` + URL scrubbing (breadcrumbs, events, **Replay**, `token_hash`, share path) + dead font imports                                                     | Scrubber extracted to `src/lib/sentry/scrub-url.ts` w/ tests                                                                                                     |
