...
3. Verify video autoplays muted on page load (or poster shows if video files missing)
4. Scroll down until hero is less than 30% visible
5. Verify video pauses when scrolled out of view
6. Scroll back up - verify video resumes playing
7. Toggle reduced motion in OS settings - verify poster image displays instead of video
8. Run `pnpm typecheck` - no errors
  </verify>
  <done>
