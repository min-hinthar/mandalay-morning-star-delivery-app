...
must_haves:
  truths:
    ...
    - "Menu section uses UnifiedMenuItemCard from Phase 18"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/components/homepage/HomepageMenuSection.tsx"
      to: "UnifiedMenuItemCard"
      via: "import and render"
      pattern: "UnifiedMenuItemCard"
---
...
<task type="auto">
  <name>Task 3: Verify Menu Section Uses UnifiedMenuItemCard</name>
  <files>src/components/homepage/HomepageMenuSection.tsx</files>
  <action>
Verify that HomepageMenuSection correctly uses UnifiedMenuItemCard from Phase 18:

1. Check import: `import { UnifiedMenuItemCard } from "@/components/menu/UnifiedMenuItemCard"`
2. Verify UnifiedMenuItemCard is rendered for each menu item (not old MenuItemCard)
3. If not using UnifiedMenuItemCard, update to use it with proper props

This integration was completed in Phase 18 (18-02-PLAN.md). This task confirms the wiring is still correct after any Phase 19 changes.

No changes needed if already using UnifiedMenuItemCard correctly.
  </action>
  <verify>
Run grep to confirm UnifiedMenuItemCard usage:
```bash
grep -n "UnifiedMenuItemCard" src/components/homepage/HomepageMenuSection.tsx
```
Should show import and usage. If not found, integration is broken.
  </verify>
  <done>
Menu section confirmed using UnifiedMenuItemCard from Phase 18.
  </done>
</task>
...
- Menu section using UnifiedMenuItemCard from Phase 18
  </what-built>
  <how-to-verify>
1. Run `pnpm dev`
...
10. Verify Menu section shows UnifiedMenuItemCard design (glassmorphism, 3D tilt on hover)
  </how-to-verify>
...
Menu section verified using UnifiedMenuItemCard from Phase 18.
