...
The `FLOATING_FOOD_ITEMS` array (lines 56-125) uses `zIndex: 1|2|3` for local stacking within the component. Add disable above the array:

```tsx
// Local stacking context values for floating food layering - not global z-index
// eslint-disable-next-line no-restricted-syntax
const FLOATING_FOOD_ITEMS: FloatingFoodItem[] = [
```

Note: ESLint disable-next-line only covers the line after it. For a multi-line array, use block disable:
```tsx
...
Hero.tsx uses `isolate` class creating a local stacking context. Add inline disables for specific lines:
...
Line 270 (motion.div zIndex: 3) - has two violations on same line:
```tsx
// eslint-disable-next-line no-restricted-syntax
...
- [ ] TypeScript strict flags enabled (from 13-01)
