...
must_haves:
  truths:
    - "App detects device capability on load (memory + cores)"
 ...
 * - low: <=4 GB memory OR <=4 CPU cores OR mobile Safari
 * - high: >4 GB memory AND >4 CPU cores (or desktop Safari)
 */
...
/**
 * Detects device capability based on hardware specs.
...
2. AnimationProvider wraps app and provides context
3. useAnimationContext accessible from any component
4. isParallaxEnabled returns false on low-power devices
5. All existing animations continue working (no regressions)
</success_criteria>
