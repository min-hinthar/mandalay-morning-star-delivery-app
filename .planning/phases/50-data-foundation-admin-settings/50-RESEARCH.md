...
export function SaveButton({ hasChanges, saving, showSuccess, disabled, onClick }: SaveButtonProps) {
  return (
    <Button
    ...
    >
      <AnimatePresence mode="wait">
        {saving ? (
          <m.span key="saving" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <m.span key="save" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
...
export function UnsavedChangesBar({ hasChanges, saving, onSave, onDiscard }: UnsavedChangesBarProps) {
  return (
    <AnimatePresence>
      {hasChanges && (
        <m.div
