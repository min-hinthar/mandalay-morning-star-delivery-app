...
4. Other driver components: Standard surface/text token migration
   ...
   <verify>
5. Run `grep -E "text-white|text-black|bg-white|bg-black" src/components/driver/*.tsx` - only matches should have eslint-disable comments
6. Run `grep -E "text-white|text-black|bg-white|bg-black" src/components/layouts/DriverLayout.tsx` - matches should have eslint-disable comments
   </verify>
