...
- `src/components/layout/CommandPalette/` (search)
  ...
  <verify> - `ls src/components/layout/header.tsx` returns "No such file" - `ls src/components/layout/MobileNav.tsx` returns "No such file" - `pnpm typecheck` passes
  </verify>
...
<verify> - `pnpm typecheck` passes - `pnpm build` passes - `pnpm knip 2>&1 | grep "Unused files"` shows reduced count
</verify>
