...
- src/components/menu/MenuEmptyState.tsx
  ...
  <verify> - `grep -r "from.*@/lib/animations" src/` returns no results - `grep -r "from.*@/lib/motion\"" src/` returns no results (note: motion-tokens still valid) - `ls src/lib/animations.ts` returns "No such file" - `ls src/lib/motion.ts` returns "No such file" - `pnpm typecheck` passes
  </verify>
