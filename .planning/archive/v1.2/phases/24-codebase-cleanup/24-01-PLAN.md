...
<verify> - `ls src/components/3d/` returns "No such file or directory" - `ls src/app/(dev)/3d-test/` returns "No such file or directory" - `ls src/components/homepage/Hero3DSection.tsx` returns "No such file or directory" - `pnpm typecheck` passes
</verify>
...
4. Verify packages removed from package.json
   ...
   <verify> - `ls public/models/` returns "No such file or directory" - `grep -c "react-three" package.json` returns 0 - `grep -c '"three"' package.json` returns 0 - `grep -c "detect-gpu" package.json` returns 0 - `pnpm build` passes
   </verify>
