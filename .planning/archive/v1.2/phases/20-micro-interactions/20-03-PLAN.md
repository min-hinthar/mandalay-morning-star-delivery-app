...
must_haves:
  ...
  key_links:
    - from: "src/components/ui-v8/cart/QuantitySelector.tsx"
      ...
      pattern: "import.*spring.*from.*motion-tokens|spring\\.rubbery"
---
...
1. Import spring.rubbery from motion-tokens.ts (already has: stiffness: 350, damping: 8, mass: 1)
   Use: `import { spring } from "@/lib/motion-tokens"`
...
export function AnimatedImage({
  ...
  if (!shouldAnimate) {
    return <Image className={className} alt={alt} onLoad={handleLoad} {...imageProps} />;
  }
  ...
  return (
    ...
    >
      <Image className="w-full h-full object-cover" alt={alt} onLoad={handleLoad} {...imageProps} />
    </motion.div>
  );
}
