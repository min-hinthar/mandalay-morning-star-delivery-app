...
must_haves:
  truths:
    - "GSAP scroll choreography tools are available for scroll-triggered animations"
    ...
    - "Parallax effect tools available for use in creative page layouts"
  artifacts:
...
Purpose: Deliver the creative effects toolkit - scroll-triggered reveals, parallax layers, and page transitions that enable the "delightfully alive with motion" experience. These are building blocks for future creative layouts.
...
<task type="auto">
  <name>Task 1: Create ScrollChoreographer and RevealOnScroll components</name>
  <files>
    src/components/ui-v8/scroll/ScrollChoreographer.tsx
    src/components/ui-v8/scroll/RevealOnScroll.tsx
  </files>
  <action>
Create scroll directory with orchestration and reveal components:
...
Implementation using useGSAP with scope for proper cleanup. Direction determines initial x/y offset.

Both components marked "use client" - require browser APIs.
</action>
<verify>

- Both files exist in src/components/ui-v8/scroll/
- Both use useGSAP with scope for cleanup
- Both import from @/lib/gsap (not directly from gsap)
- ScrollChoreographer uses gsap.utils.toArray
- RevealOnScroll supports direction prop
  </verify>
  <done>
...
- Both properly clean up on unmount via useGSAP scope
  </done>
  </task>

<task type="auto">
  <name>Task 2: Create ParallaxLayer component and scroll barrel export</name>
  <files>
    src/components/ui-v8/scroll/ParallaxLayer.tsx
    src/components/ui-v8/scroll/index.ts
  </files>
  <action>
Create ParallaxLayer and barrel export:
...
```
  ...
  <verify>
- ParallaxLayer.tsx exists with speed and direction props
- index.ts exports all three scroll components
- ParallaxLayer uses scrub: 1 for smooth parallax
- All imports from @/lib/gsap (not direct gsap import)
  </verify>
  <done>
- ParallaxLayer enables parallax effects with configurable speed
- Barrel export includes all scroll components
- All components use useGSAP scope for cleanup
  </done>
</task>

<task type="auto">
  <name>Task 3: Create enhanced PageTransitionV8 component</name>
  <files>
    src/components/ui-v8/transitions/PageTransitionV8.tsx
    src/components/ui-v8/transitions/index.ts
  </files>
...
grep "from._@/lib/gsap" src/components/ui-v8/scroll/_.tsx
! grep "from \"gsap\"" src/components/ui-v8/scroll/\*.tsx || echo "Found direct gsap import - FIX"

# Verify useGSAP with scope

grep "useGSAP._scope" src/components/ui-v8/scroll/_.tsx
