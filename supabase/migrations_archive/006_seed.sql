-- ===========================================
-- 006: Seed Data
-- Menu (from data/seed-menu.sql), featured sections, app settings
-- All ON CONFLICT for idempotency
-- ===========================================
...
-- ===========================================
-- 3. MODIFIER OPTIONS (group-prefixed slug convention)
-- ===========================================
...
-- ===========================================
-- 4. MENU ITEMS (sourced from data/seed-menu.sql)
-- ===========================================
...
-- ===========================================
-- 6. FEATURED SECTIONS (predefined)
-- ===========================================
...
-- ===========================================
-- 7. APP SETTINGS (correct final values)
-- Consolidated from 010, 019, 020, 023, 025, 029, 035
-- ===========================================
...
INSERT INTO app_settings (key, value, category) VALUES
  ...
  ('free_delivery_threshold_cents',  '10000'::jsonb, 'delivery'),
  ('base_delivery_fee_cents',        '1500'::jsonb,  'delivery'),
  ...
  ('prep_time_buffer_minutes',       '30'::jsonb,    'delivery')
...
-- Operations: admin contact info (with description)
INSERT INTO app_settings (key, value, category, description) VALUES
  ('admin_contact_info', '{"email": "admin@morningstar.com", "phone": ""}'::jsonb, 'operations', 'Contact information shown to deactivated drivers')
ON CONFLICT (key) DO UPDATE SET
  value = EXCLUDED.value,
  category = EXCLUDED.category,
...
INSERT INTO app_settings (key, value, category) VALUES
  ...
  ('email_sending_enabled',           'true'::jsonb,  'notifications')
ON CONFLICT (key) DO UPDATE SET
  value = EXCLUDED.value,
  category = EXCLUDED.category;
...
BEGIN
  ...
  RAISE NOTICE 'Seed complete: % categories, % items, % modifier groups, % modifier options, % app settings',
    cat_count, item_count, mod_group_count, mod_option_count, settings_count;
END $$;
